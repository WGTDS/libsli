...
static void _null_smsr00_blocks( u8 *def, u16 *pol, u16 *cmd )
{
  if ( def != (u8 *)0 )
  {
    free( def );
...
static u8 *_produce_smsr00( u8 *src, u8 **dst,
                            const u32 srclen, u32 **dstlen )
{
  ...
  cmd = calloc( (0x2000U << ENC_GREED), sizeof(u8) );
  pol = malloc( (npp *  2U) );
  def = malloc( (ndp << 2) );
  srcpos = 0;
  mask   = 0x8000U;
  ...
  *((u32 *)(*dst + 0x00U)) = _swap32( (u32)CMPR );
  *((u32 *)(*dst + 0x04U)) = _swap32( **dstlen );
  *((u32 *)(*dst + 0x08U)) = _swap32(  srclen );
  *((u32 *)(*dst + 0x0CU)) = _swap32( 0 );
  *((u32 *)(*dst + 0x10U)) = _swap32( (u32)SMSR );
  *((u32 *)(*dst + 0x14U)) = _swap32( (u32)0x30300000U );
  *((u32 *)(*dst + 0x18U)) = _swap32( srclen );
  *((u32 *)(*dst + 0x1CU)) = _swap32( adr );
  ...
  while ( 1 )
  {
    ...
    {
      flags = (i32)cmd[i++];
      *((u16 *)(*dst + (0x20U + adr))) = _swap16( (u16)flags );
      flags <<= 0x10;
      masks = 16U;
      adr  += 2U;
    }
    else
    {
      if ( flags >= 0 )
      {
        *((u16 *)(*dst + (0x20U + adr))) = _swap16( pol[j++] );
        adr += 2U;
      }
  ...
  {
    *(*dst + ((0x20U + adr) + srcpos)) = def[srcpos];
  }

  _null_smsr00_blocks( def, pol, cmd );
  return *dst;
...
u8 *_encode_smsr00( u8 *src, u8 **dst,
                    const u32 srclen, u32 *dstlen )
{
  return *dst = _produce_smsr00( src, &*dst, srclen, &dstlen );
}
