...
static void _null_yay0_blocks( u8 *def, u16 *pol, u32 *cmd )
{
  if ( def != (u8 *)0 )
  {
    free( def );
...
static u8 *_produce_yay0( u8 *src, u8 **dst,
                          const u32 srclen, u32 **dstlen )
{
  ...
  while ( srcpos < srclen )
  {
    ...
    else
    {
      ...
      {
        pol[pp++] = ((((i16)match_length + -2) * 0x1000) | (u16)match_position);
      }
      else
      {
        pol[pp++] = (u16)match_position;
        def[dp++] = (i8)(match_length + -0x12);
...
u8 *_encode_yay0( u8 *src, u8 **dst,
                  const u32 srclen, u32 *dstlen )
{
  return *dst = _produce_yay0( src, &*dst, srclen, &dstlen );
}
