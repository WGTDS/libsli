...
static u8 *_process_yay0( u8 *src, u8 **dst )
{
  u32 word;
  u32 declen;
  u32 dsp;
  ...
  do
  {
    ...
    else
    {
      ...
      {
        dsp    = (u32)_swap16( *(u16 *)pairs );
        qty    = ((dsp >> 12) ? ((dsp >> 12) + 2U) : ((u32)*bytes++ + 18U));
...
{
  return *dst = _process_yay0( src, &*dst );
}
